...
#### ✅ ACCEPT — DS semantic colour tokens (Figma role/state system)

These are the ONLY correct classes for new DS components in `components/{Name}/`.
Pattern: `bg-{role}-{state}` / `text-{role}-{state}`

| Role | background | subtle | light | default | hover | active | emphasis | text |
|------|-----------|--------|-------|---------|-------|--------|----------|------|
| `brand` | `bg-brand-background` | `bg-brand-subtle` | `bg-brand-light` | `bg-brand-default` | `bg-brand-hover` | `bg-brand-active` | `bg-brand-emphasis` | `text-brand-text` |
| `error` | `bg-error-background` | `bg-error-subtle` | `bg-error-light` | `bg-error-default` | `bg-error-hover` | `bg-error-active` | `bg-error-emphasis` | `text-error-text` |
| `success` | `bg-success-background` | — | — | `bg-success-default` | `bg-success-hover` | — | — | `text-success-text` |
| `warning` | `bg-warning-background` | — | — | `bg-warning-default` | `bg-warning-hover` | — | — | `text-warning-text` |
| `caution` | `bg-caution-background` | — | — | `bg-caution-default` | `bg-caution-hover` | — | — | `text-caution-text` |
| `information` | `bg-information-background` | — | — | `bg-information-default` | — | — | — | `text-information-text` |
| `neutral` | `bg-neutral-background` | `bg-neutral-subtle` | `bg-neutral-light` | `bg-neutral-default` | `bg-neutral-hover` | `bg-neutral-active` | `bg-neutral-emphasis` | `text-neutral-text` |
| `teal` | `bg-teal-background` | — | — | `bg-teal-default` | `bg-teal-hover` | — | — | `text-teal-text` |
| `purple` | `bg-purple-background` | — | — | `bg-purple-default` | `bg-purple-hover` | — | — | `text-purple-text` |
| `magenta` | `bg-magenta-background` | — | — | `bg-magenta-default` | `bg-magenta-hover` | — | — | `text-magenta-text` |

All `text-{role}-{state}` variants are also valid (e.g. `text-brand-default`, `text-neutral-default`).
`border-{role}-{state}` variants are valid for borders.

#### ✅ ALSO ACCEPT — shadcn compat tokens (shadcn primitives ONLY)

These are only acceptable inside `src/components/ui/` (shadcn primitives).
**Do NOT use these in new DS components in `components/{Name}/`.**
...
Opacity modifiers are fully supported on all semantic tokens:
- `hover:bg-brand-default/90` ✅
- `bg-error-background/50` ✅
- `hover:bg-primary/90` ✅ (shadcn compat only)

#### How to find the correct semantic token for a raw hex value

1. Look up the hex in `tokens/tokens.json` — find the `tailwind` field for the correct class
2. Match the context: background fill → `bg-{role}-{state}`, text → `text-{role}-{state}`, border → `border-{role}-{state}`
3. Use the Figma token name if provided in the handoff:
   - Figma "brand/default" → `bg-brand-default`
   - Figma "error/background" → `bg-error-background`
   - Figma "neutral/text" → `text-neutral-text`
   - Figma "success/default" → `bg-success-default`
